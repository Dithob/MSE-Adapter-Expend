...
class MMDataset(Dataset):
    ...
    def __init_iemocap(self):
        ...
        if not self.args.need_data_aligned:
            self.audio_lengths = np.array(list(map(lambda item: max(1, min(item['features']['audio_len'], self.args.seq_lens[1])), data)))
            self.vision_lengths = np.array(list(map(lambda item: max(1, min(item['features']['video_len'], self.args.seq_lens[2])), data)))
